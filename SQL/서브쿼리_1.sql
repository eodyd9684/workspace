...
#사번이 1005번 사원의 사번, 사원명, 부서번호, 부서명, 부서가 위치한 지역을 조회(JOIN 사용 금지)
SELECT EMPNO
		, ENAME
		, DEPTNO
		, (SELECT DNAME
...
#댓글번호가 1번인 댓글의 댓글내용, 댓글 작성자, 해당 댓글이 달린 게시글의 제목, 내용을 조회하세요.
...
, (SELECT COUNT(REPLY_NUM)
	FROM board_reply
	WHERE BOARD_NUM = board.BOARD_NUM) AS REPLY_COUNT #COUNT(REPLY_NUM)
FROM board;
