`member`study_student`member`emp
 -- 주석
 
 -- 실행 : 현재 열려있는 쿼리 탭의 모든 쿼리를 일괄 실행
 -- 선택실행 : 마우스로 드래그 한 쿼리만 실행
...
-- EMP 테이블의 모든 사원들 중에
-- 급여가 400이하 이거나 700이상인 사원들의 사번, 사원명, 급여를 조회
...
SELECT ENAME, JOB, SAL FROM emp WHERE JOB = '대리';
...
SELECT ENAME, JOB, SAL, COMM FROM emp WHERE SAL <=900 AND COMM IS NOT NULL AND (JOB = '대리' OR JOB = '과장');
