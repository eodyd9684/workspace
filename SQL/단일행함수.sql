...
# 결론 1 : 만약, 테이블에 저장된 데이터의 갯수를 알고 싶다면 NULL데이터가
...
# 결론 : 단일행 함수는 단일행 함수끼리만 사용 가능!!! 
#			다른 일반 컬럼과 단일행 함수를 같이 사용하면 쿼리 오류!
#			단, GROUP BY 절에 작성한 컬럼은 단일행함수와 함께 사용 가능
...
#부서별 커미션의 평균을 조회
#단, 10번부서는 제외하고, 동시에 평균은 전체 사원수를 기준으로 조회
...
# 직급별 최소 급여가 300미만인 데이터도 제외한다.
# 조회 결과 데이터를 직급 기준 오름차순으로 정렬하세요.
SELECT JOB, MIN(SAL)
FROM emp
WHERE JOB != '사원'
GROUP BY JOB
